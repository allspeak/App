...
function TrainingCtrl($scope, $state, $ionicPopup, $ionicHistory, $ionicPlatform, VocabularySrv, InitAppSrv, FileSystemSrv, StringSrv, EnumsSrv)  
{
    ...
    {    
        $state.go('dotraining', {modeId:3, foldername:$scope.default_training_folder});
    }
    
    $scope.doEditTrainVocabulary = function() 
    {
        $state.go('dotraining', {modeId:1, foldername:$scope.training_folder_name});
    };
    
    $scope.startTraining = function()
    {    
        $state.go('dotraining', {modeId:2, foldername:$scope.training_folder_name});
    }
